...
export function ChatInterface({ session }: ChatInterfaceProps) {
  ...
  const handleSendMessage = async () => {
    ...
    setSending(true)
    try {
      const message = await sendChatMessage(session.id, currentUser.id, newMessage)
                  ...
                  <div key={message.id} className={`flex ${isCurrentUser ? "justify-end" : "justify-start"}`}>
                    <div className={`flex gap-2 max-w-[80%] ${isCurrentUser ? "flex-row-reverse" : "flex-row"}`}>
                      <Avatar className="h-8 w-8">
                        ...
                        <AvatarFallback>
                          {isCurrentUser ? currentUser?.name.substring(0, 2).toUpperCase() : "EX"}
                        </AvatarFallback>
                      </Avatar>
                      <div>
                        <div
                        ...
                        <p className={`text-xs mt-1 ${isCurrentUser ? "text-right" : ""}`}>

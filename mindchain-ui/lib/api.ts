import type { Query, User, ChatSession, Notification, MatchedExpert, ChatMessage } from "./types"
...
const mockChatSessions: ChatSession[] = [
  {
    ...
    messages: [
      ...
      },
      {
        ...
        sessionId: "cs1",
        senderId: "u1",
        content: "Hi Rahul, I would be happy to help. Can you describe the issue you are facing?",
      ...
      },
      {
        ...
        sessionId: "cs1",
        senderId: "u3",
        content: "I am working with a dataset for image classification and my model accuracy is stuck at 70%.",
...
export async function submitQuery(
  query: Omit<Query, "id" | "createdAt" | "updatedAt" | "status" | "matchedExperts">,
): Promise<Query> {
  await new Promise((resolve) => setTimeout(resolve, MOCK_DELAY))
...
export async function getChatSessionByQuery(queryId: string): Promise<ChatSession | undefined> {
  await new Promise((resolve) => setTimeout(resolve, MOCK_DELAY))
  return mockChatSessions.find((session) => session.queryId === queryId)
}

export async function sendChatMessage(sessionId: string, senderId: string, content: string): Promise<ChatMessage> {
  await new Promise((resolve) => setTimeout(resolve, MOCK_DELAY))
...
export async function acceptQuery(queryId: string, expertId: string): Promise<boolean> {
  await new Promise((resolve) => setTimeout(resolve, MOCK_DELAY))
